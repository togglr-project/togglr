...
for conflist in $(find $NGINX_PATH/templates -name "*.template" -exec basename {} \;); do
    envsubst '${DOMAIN},${BACKEND_URL},${FRONTEND_URL},${PG_ACL_LIST},${SSL_CERT},${SSL_CERT_KEY},${SECURE_LINK_MD5}' < "$NGINX_PATH"/templates/"$conflist" > "$NGINX_PATH"/conf.d/"${conflist%.*}".conf
done
