...
func (r *Repository) ListByProjectIDWithEnvID(
	ctx context.Context,
	...
	const query = `SELECT * FROM feature_algorithms WHERE environment_id = $1 AND project_id = $2`

	rows, err := executor.Query(ctx, query, envID, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
