...
select 
    coalesce(recent.cnt, 0) as cnt,
    coalesce(last_err.last_at, to_timestamp(0)) as last_at,
    (coalesce(recent.cnt, 0)::double precision / nullif($3,0)) as error_rate
from recent 
left join last_err on true;`

	secs := int(window.Seconds())
	if err := exec.QueryRow(ctx, query, featureID, envID, secs).Scan(&cnt, &lastErr, &errorRate); err != nil {
