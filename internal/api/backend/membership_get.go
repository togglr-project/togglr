...
func (r *RestAPI) GetProjectMembership(
	...
	membership, err := r.membershipsUseCase.GetProjectMembership(ctx, projectID, domain.MembershipID(params.MembershipID.String()))
	if err != nil {
		if errors.Is(err, domain.ErrEntityNotFound) {
			return &generatedapi.ErrorNotFound{Error: generatedapi.ErrorNotFoundError{Message: generatedapi.NewOptString("not found")}}, nil
		}
